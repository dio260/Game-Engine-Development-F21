...
namespace CPI311.GameEngine
{
    public class ParticleManager
    {
        ...
        public ParticleManager(GraphicsDevice g, int max)
        {
            ...
            {
                vtx[i * 4 + 0] =
            new VertexInfo(new Vector4(-500, -500, 0, 1), new Vector2(0, 1));
                vtx[i * 4 + 1] =
            new VertexInfo(new Vector4(-500, 500, 0, 1), new Vector2(0, 0));
                vtx[i * 4 + 2] =
            new VertexInfo(new Vector4(500, -500, 0, 1), new Vector2(1, 1));
                vtx[i * 4 + 3] =
            new VertexInfo(new Vector4(500, 500, 0, 1), new Vector2(1, 0));
            }
            vertexBuffer =
        new DynamicVertexBuffer(g, typeof(VertexInfo), max * 4, BufferUsage.WriteOnly);
            ...
            indexBuffer =
            new IndexBuffer(g, typeof(ushort), idx.Length, BufferUsage.WriteOnly);
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < particleMax; i++)
            {
                ...
                if (particles[i].IsActive())
                {
                    ...
                    vtx[ax + 0].param.X = vtx[ax + 1].param.X = vtx[ax + 2].param.X =
                    vtx[ax + 3].param.X = pp.Age;
                    vtx[ax + 0].param.Y = vtx[ax + 1].param.Y = vtx[ax + 2].param.Y =
                    vtx[ax + 3].param.Y = pp.MaxAge;
                    activeMax++;
                }
            }
        }
        public void Draw(GraphicsDevice g)
        {
            ...
            g.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, 0,
                                activeMax * 4, 0, activeMax * 2);
        }
    }
